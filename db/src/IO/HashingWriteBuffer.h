...
template <typename T>
class IHashingBuffer : public BufferWithOwnMemory<T>
{
public:
    using uint128 = CityHash_v1_0_2::uint128;

    IHashingBuffer<T>(size_t block_size_ = DBMS_DEFAULT_HASHING_BLOCK_SIZE)
        : BufferWithOwnMemory<T>(block_size_), block_pos(0), block_size(block_size_), state(0, 0)
    {
    }
    ...
    {
        if (block_pos)
            return CityHash_v1_0_2::CityHash128WithSeed(&BufferWithOwnMemory<T>::memory[0], block_pos, state);
        else
            return state;
    }
...
class HashingWriteBuffer : public IHashingBuffer<IO::WriteBuffer>
{
private:
    WriteBuffer & out;
...
public:
    HashingWriteBuffer(
        ...
        : IHashingBuffer<IO::WriteBuffer>(block_size_), out(out_)
    {
