...
int main ()
{
    std::string queryString ;// = "select * from db.table where name = 1";
    ReadBufferFromFileDescriptor in(STDIN_FILENO);
    WriteBufferFromFileDescriptor out(STDOUT_FILENO);
    readStringUntilEOF(queryString, in);
    Lexer lexer(queryString.data(), queryString.data() + queryString.size());
    while (true)
    {
