...
inline void writeHexByteUppercase(Poco::UInt8 byte, void * out)
{
    memcpy(out, &hex_byte_to_char_uppercase_table[static_cast<size_t>(byte) * 2], 2);
}

inline void writeHexByteLowercase(Poco::UInt8 byte, void * out)
{
...
{
    return hex_char_to_digit_table[static_cast<Poco::UInt8>(c)];
}

inline char unhex2(const char * data)
{
    return
        static_cast<Poco::UInt8>(unhex(data[0])) * 0x10
        + static_cast<Poco::UInt8>(unhex(data[1]));
}

inline Poco::UInt16 unhex4(const char * data)
{
    return
        static_cast<Poco::UInt16>(unhex(data[0])) * 0x1000
        + static_cast<Poco::UInt16>(unhex(data[1])) * 0x100
        + static_cast<Poco::UInt16>(unhex(data[2])) * 0x10
        + static_cast<Poco::UInt16>(unhex(data[3]));
}
